...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    if (!process_capsnum(keycode, record)) {
        return false;
    }
    if (!process_esc_to_base(keycode, record)) {
        return false;
    }
    if (!process_lsft_for_caps(keycode, record)) {
        return false;
    }
    ...
    switch (keycode) {
        ...
        case PRNCONF: // Print verbose status of all user_config toggles (open a text editor before engaging!!)
            if (record->event.pressed) {
                ...
                send_string(" joncrangle" SS_LALT(SS_TAP(X_KP_0) SS_TAP(X_KP_0) SS_TAP(X_KP_3) SS_TAP(X_KP_9)) "s GMMK Pro User Settings ");
